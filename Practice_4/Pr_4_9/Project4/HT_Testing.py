...
def test():
    ...
    hash_table = HT()  # Создание хеш-таблицы.
    comb = Combining(hash_table, fw)  # Объединение работы файла и хеш-таблицы.
    ...
    print(f'Количество сравнений при получении первой записи: {Hash_Table.compares}')
    print()
    print(f'Последняя запись из файла: {comb.get(str(numbers[9999]))}')
    print(f'Количество сравнений при получении последней записи: {Hash_Table.compares}')
    print()
    print(f'Запись в середине файла: {comb.get(str(numbers[4999]))}')
    print(f'Количество сравнений при получении записи в середине файла: {Hash_Table.compares}')
