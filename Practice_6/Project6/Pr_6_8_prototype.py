...
class Graph:
    ...
    def connect(self, node1, node2, weight):
        ...
        self.adj_mat[node1][node2] = weight  # Вставка ребра в матрицу смежности для алгоритма Флойда-Уоршелла.
    ...
    def shortest_path_floyd(self, node1, node2):  # Для заданного узла node1 и другого заданного узла node2:
        return self.floyd()[node1][node2]  # Возврат величины кратчайшего пути между node1 и node2.
    ...
    def bfs(self, node):
        ...
        while len(nodes_to_visit):  # Пока nodes_to_visit не пустой:
            weight, current_node = nodes_to_visit[0]  # Выбор следующей вершины.
    ...
    def shortest_path_bfs(self, node1, node2):  # Для заданного узла node1 и другого заданного узла node2:
        return self.bfs(node1)[node2]  # Возврат величины кратчайшего пути между node1 и node2.
    ...
    def app_shortest_path(self, hor_edges, vert_edges):  # Для заданных списков горизонтальных и вертикальных рёбер:
        ...
        print('Рассмотрим алгоритм обхода "в ширину" для поиска кратчайшего пути.')
        print(f'Величина кратчайшего пути: {self.shortest_path_bfs(start_node, end_node)}.')
        print("Рассмотрим алгоритм Дейкстры для поиска кратчайшего пути.")
        print(f'Величина кратчайшего пути: {self.shortest_path_dijkstra(start_node, end_node)}.')
        print("Рассмотрим алгоритм Флойда-Уоршелла для поиска кратчайшего пути.")
        print(f'Величина кратчайшего пути: {self.shortest_path_floyd(start_node, end_node)}.')
...
if __name__ == '__main__':
    ...
    if input_type == '1':  # При вводе '1' данные будут введены вручную:
